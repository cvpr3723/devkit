...
from auxiliary.visualizers import BoundingBoxVisualizer
...
def main():
  ...
  pred_fnames = get_txt_files_in_dir(args['pred_dir'])
  ...
  for img_fname, pred_fname in tqdm(zip(img_fnames, pred_fnames), total=n_total):
    assert img_fname.split('.')[0] == pred_fname.split('.')[0]

    img = np.array(Image.open(args['image_dir'] / img_fname))
    pred = np.loadtxt(args['pred_dir'] / pred_fname)
